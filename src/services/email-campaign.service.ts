...
export class EmailCampaignService {
  // Map to store campaignId -> current executionId
  private activeCampaigns = new Map<string, string>();

  /**
   * Check if a campaign is currently running in memory
   */
  ...
  async sendCampaign(campaignId: string) {
    ...
    this.activeCampaigns.set(campaignId, executionId);
    ...
    try {
      ...
      // Helper to sleep with periodic database checks to abort if STOPPED or superseded
      const sleepWithCheck = async (minutes: number): Promise<boolean> => {
        const ms = minutes * 60 * 1000;
        const checkInterval = 5000; // Check DB every 5 seconds
        let elapsed = 0;
        while (elapsed < ms) {
          if (this.activeCampaigns.get(campaignId) !== executionId) {
            console.log(`Campaign ${campaignId}: execution superseded during sleep. Exiting.`);
            return false;
          }
      ...
      const processRecipient = async (recipient: any, index: number) => {
        // Determine which sender to use (Round Robin) - continue from where we left off
        const senderIndex = (totalSentAtStart + index) % transporters.length;
      ...
      for (let idx = 0; idx < campaign.recipients.length; idx++) {
        ...
        // Check if campaign was manually stopped/cancelled or superseded
        if (this.activeCampaigns.get(campaignId) !== executionId) {
          console.log(`Campaign ${campaignId}: execution superseded in loop. Halting sending.`);
      ...
      // Only remove if this was the current execution (not replaced by a new one)
      if (this.activeCampaigns.get(campaignId) === executionId) {
